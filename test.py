...
def plot_std_comparison(std_data, toxin, save_path=None, stat_test='bootstrap', stats_median=False):
    """
    ...
    stats_median : bool
        If True, use bootstrap on medians; if False, use bootstrap on means
    """
        ...
        if len(type_data) > 0:
            ...
            if len(pre_data) > 0 and len(post_data) > 0:
                ...
                if stat_test == 'bootstrap':
                    ...
                    if stats_median:
                        p_value = statsf.bootstrap_test(pre_data, post_data, function=np.median)[0]
                    else:
...
def plot_event_rate_comparison(event_data, toxin, save_path=None, stat_test='bootstrap', stats_median=False):
    """
    ...
    for i, data_type in enumerate(['voltage', 'calcium']):
        ...
        if len(type_data) > 0:
            ...
            if len(pre_data) > 0 and len(post_data) > 0:
                ...
                if stat_test == 'bootstrap':
                    if stats_median:
                        p_value = statsf.bootstrap_test_2sided(pre_data, post_data, function=np.median)[0]
                    else:
...
def save_event_rate_info(event_data, toxin, save_dir, stats_median=False):
    """
    ...
    stats_median : bool
        If True, report median + IQR and use bootstrap on medians
    """
    if stats_median:
        info_path = save_dir / f'events_info_{toxin}_median.txt'
    else:
        ...
        for data_type in ['voltage', 'calcium']:
            ...
            for segment in ['pre', 'post']:
                ...
                if len(segment_data) > 0:
                    ...
                    if stats_median:
                        median_rate = segment_data['event_rate_per_100s'].median()
                        q25 = segment_data['event_rate_per_100s'].quantile(0.25)
                        q75 = segment_data['event_rate_per_100s'].quantile(0.75)
                        iqr = q75 - q25
                    ...
                    if stats_median:
                        f.write(f"  Median event rate: {median_rate:.3f} (IQR: {iqr:.3f}) events/100s\n")
            ...
            if len(pre_data) > 0 and len(post_data) > 0:
                if stats_median:
                    p_value = statsf.bootstrap_test_2sided(pre_data, post_data, function=np.median)[0]
                    f.write(f"  P-value (bootstrap on medians, 2-sided): {p_value:.6f}\n")
                else:
                    p_value = statsf.bootstrap_test_2sided(pre_data, post_data)[0]
...
def analyze_toxin(toxin, save_dir=None, plot_std=True, plot_event_rate=True, include_post_folders=False, stats_median=False):
    """
    ...
    stats_median : bool
        If True, use median-based statistics and reporting
    """
    print(f"\n{'='*70}")
    ...
    if stats_median:
        print(f"  (stats_median=True: using median-based statistics)")
    print(f"{'='*70}")
    ...
    if save_dir:
        ...
        # Create median subfolder if stats_median is True
        if stats_median:
            save_dir = save_dir / 'median'
        save_dir.mkdir(parents=True, exist_ok=True)
    ...
    if plot_std and std_data is not None and len(std_data) > 0:
        plot_std_comparison(std_data, toxin, std_save_path, stats_median=stats_median)
    elif plot_std:
        print(f"No standard deviation data found for {toxin}")
    
    if plot_event_rate and event_data is not None and len(event_data) > 0:
        plot_event_rate_comparison(event_data, toxin, event_save_path, stats_median=stats_median)
        
        # Save event rate info to text file
        if save_dir:
            save_event_rate_info(event_data, toxin, save_dir, stats_median=stats_median)
...
if __name__ == "__main__":
    ...
    for toxin in toxins:
        try:
            ...
            # Set stats_median=True to use median-based statistics and reporting
